...
- 首包 bootstrap 只允许 package 尚不存在且 `NPM_BOOTSTRAP_GIT_SHA` 等于已批准 commit；临时 `NPM_TOKEN` 只进入该 publish step。包存在后 bootstrap 永久被代码拒绝，后续必须使用 Trusted Publishing OIDC。rc.5 的 run `31949610430` 已完成 build 与两类 attestation，并创建包含六个附件的 Draft Release `371332167`，但 `stage_draft` 随后通过 tag endpoint 查询未发布 Draft 时收到 404，因此 npm package 仍不存在。该 run、tag 和 Draft 必须保留，不能重跑、移动、删除或复用。
- rc.6 run `31950986283` 已证明分页 Release 列表修复有效：build、attestation 与 Draft `371339257` 的六附件暂存均成功。受保护 Environment 放行后，`publish_npm` 以 `contents: read` 按 Release ID 复核同一 Draft 时返回 403，因此 npm pin、preflight、publish、provenance 与 finalize 全部未执行；npm package 仍不存在。rc.6 的 run、tag、Draft 与附件必须保留，不能重跑、移动、删除或复用。
- 当前工作树继续保留本地 `link:../deepseek-harness/...` 依赖用于源码联调；它们不会进入预览 tarball。rc.7 候选保留 npm 前 Draft 即时复核，并把 `publish_npm` 提升为 job 级 `contents: write`；GitHub Actions 没有 step 级权限。精确权限、显式 `GH_TOKEN` 位置和该 job 禁止 GitHub 写命令均由 mutation 合同约束。用户已授权完成源码、测试、文档并推送 `master`，但尚未授权创建 rc.7 tag 或触发正式 workflow。
- rc.7 远程验收完成前不得声称 npm 或 GitHub Prerelease 已发布。源码 Profile 的既有 boot 冒烟与用户真机 `/ls` 验收、tarball-backed 真实 Profile 启动/HTTP smoke、以及远程供应链验收是不同证据；真实飞书消息与故障矩阵仍不能由配置或 HTTP 代替。
