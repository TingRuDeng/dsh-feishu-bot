...
- 当前工作树继续保留本地 `link:../deepseek-harness/...` 依赖用于源码联调；它们不会进入预览 tarball。rc.6 修复改为 checkout 精确触发提交，从分页 Release 列表中 fail-closed 选择唯一 Draft。用户已授权提交、推送新的 rc.6 源码与 tag，并观察正式 workflow。
- rc.6 完成前不得声称 npm 或 GitHub Prerelease 已发布。源码 Profile 的既有 boot 冒烟与用户真机 `/ls` 验收、tarball-backed 真实 Profile 启动/HTTP smoke、以及远程供应链验收是不同证据；真实飞书消息与故障矩阵仍不能由配置或 HTTP 代替。
