...
- 当前工作树继续保留本地 `link:../deepseek-harness/...` 依赖用于源码联调；它们不会进入预览 tarball。rc.8 候选保留 npm 前 Draft 即时复核和 `publish_npm` 的受约束 job 级 `contents: write`，把 manifest、bootstrap/OIDC publish、单调性与发布后验收统一切换到 `latest`，并对 version endpoint 与根 packument 共用可测试的有界 retry。`next` 已退役并通过一次性交互认证从远端删除；公开 registry 当前只保留 `latest=0.1.0-rc.7`，临时登录已注销并清理。
...
- rc.8 远程验收完成前不得声称 GitHub Prerelease 已发布或纯 OIDC 已通过。源码 Profile 的既有 boot 冒烟与用户真机 `/ls` 验收、tarball-backed 真实 Profile 启动/HTTP smoke、rc.7 npm 供应链验收、以及 rc.8 OIDC/latest-only 验收是不同证据；真实飞书消息与故障矩阵仍不能由配置或 HTTP 代替。
