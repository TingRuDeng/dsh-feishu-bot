...
- M4 前三项已按 TDD 实现：`/ls` 快照 TTL、终态结果卡 + 24KB 完整 envelope 容量预检分片、本地路径链接改写。
...
1. `[label](/local/path)` 与 `[label](</path with spaces>)` 在分片前改写为 ``label（`/path`）``，避免飞书渲染死链接。
2. HTTP 等非本地链接保持原样；路径内反引号替换为 `ˋ`，防止提前闭合代码片段。
3. 改写后的正文参与 24KB envelope 预检，结果卡失败时的纯文本降级使用同一份可读正文。
