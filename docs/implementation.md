...
- DSH 当前兼容基线已记录：官方 `0.1.2-alpha.2`，`master` commit `0a53fb55bea101816fa226bb964ae2bed71c343b`（2026-08-31）；本地源码联调与正式 workflow 使用同一干净 checkout；
...
2026-08-17 阶段 6.7 本地与远程事实：rc.7 保留 npm 前 Draft 即时、fail-closed 复核，并把 `publish_npm` 调整为 job 级 `actions: read`、`contents: write`、`id-token: write`。GitHub Actions 没有 step 级权限；SHA-pinned checkout 使用但不持久化 token，`GH_TOKEN` 只显式进入队列和 Draft/tag 复核，mutation 合同拒绝该 job 中的 GitHub REST/CLI 写操作与 Git push。源码提交与 annotated tag 已推送，run `31989507142` 的 build、双 attestation、六附件 Draft 和 npm bootstrap publish 成功；npm 版本接口与 tarball 已就绪时，首次包根 packument 短暂返回 404，workflow 因而失败并跳过 GitHub finalize。npm 与 Draft tarball SHA-256 均为 `e0c4f8d99261f743338a98e49dfbf114c764201fdf179210c174df7a2e6a182d`，GitHub build/SBOM attestation 与 npm provenance 已独立验证。rc.7 npm 包存在，GitHub Release 仍是 Draft，全部远程事实保留。
...
## M7：模型选择与推理强度（M7.0–M7.3 全部实现，2026-08-18）
...
| M7.1 ✅（2026-08-18） | `/model` 三层卡片 | 复用 `/ls` token/TTL/operator/messageId 校验；provider→model→effort 导航/返回/分页；换 model 后档位重校验三分支（保留/回落默认/清空）；无档位 route 直接清空生效；目录/元数据失败不改选择；四条所有权拒绝路径 |
| S4 ✅（2026-08-18） | Markdown 转义统一（M7.1 前置） | 共享 `lark-markdown.ts`；审批/任务/会话列表/结果卡及 `/status` `/effort` `/ls` fallback `/unknown` `/new` 文本路径全部转义；对抗输入回归 |
